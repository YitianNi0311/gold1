...
已完成新旧入口逐项对照，详见 `零FCNN运行逻辑核对.md`：两阶段五折主流程一致，但新模型未固定 Optuna sampler seed，与目前保留的旧运行器有效配置有差异，不能保证正式训练预测相同。
...
- `TimeSeriesSplit(n_splits=5)`；先在五折产生 OOF 元特征并训练 GBM，再在相同五折重新训练基模型并评估。
- LightGBM 每个任务、每折、每个阶段均执行原 50 trials，目标内部 `cv=3`；完整运行共 1,000 trials，没有缩减搜索预算。
...
- `fold1_predictions.csv` 至 `fold5_predictions.csv`、`predictions.csv`：分类预测、标准化回归预测和原始价格预测；原价列以 USD/oz 解释。
- `scaler_y_fold1.json` 至 `scaler_y_fold5.json`：各训练折 y scaler 的均值和尺度。
- `fold_metrics.csv`：各折指标，回归键为 `rmse_usd_per_oz`、`mape_percent`。
- `metric_summary.csv`：五折均值及总体标准差（ddof=0）。
- `pooled_metrics.json`：合并样本指标，单独保存以免与五折 RMSE 均值混淆。
...
本次验证包含小型模拟数据上的完整两阶段流程、真实 GBM 元模型拟合、三个基模型的构造参数、尺度恢复和 CSV 输出；耗时的基模型搜索使用模拟替身。尚未对 GOLD 数据启动正式训练。
