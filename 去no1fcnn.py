...
def objective_lgbm_reg(trial, X_train, y_train):
    param = {
        ...
        'min_data_in_leaf': trial.suggest_int('min_data_in_leaf', 20, 100),  # Added to reduce warnings
        'min_gain_to_split': trial.suggest_loguniform('min_gain_to_split', 0.001, 1.0),  # Added to reduce warnings
        'random_state': 42,
        'n_jobs': -1
    }
...
def generate_meta_features_clf(fcnn2, lgbm_clf, xgb_clf, rf_clf, test_loader, X_test_scaled, device):
    ...
    with torch.no_grad():
        for X_batch, _ in test_loader:
            ...
            preds2.extend(torch.softmax(output2, dim=1).cpu().numpy())  # Convert to probabilities
...
def main():
    ...
    for fold, (train_index, test_index) in enumerate(tscv.split(X)):
        ...
        # Plot loss curves for Regression
        plot_loss_curve(loss_history2_reg, fold, task='regression', eval=True)
        ...
        # Plot loss curves for Classification
        plot_loss_curve(loss_history2_clf, fold, task='classification', eval=True)
